"""
AI 内容创作系统 - EdgeOne Makers Serverless 入口文件
文件名 [[default]].py 是 EdgeOne Pages 的 catch-all 动态路由语法，
映射 example.com/* 全部路径。运行时会自动剥离文件系统路由前缀后再交给
...
# 注意：不写 if __name__ == '__main__' + app.run()
# EdgeOne Serverless 运行时会直接找到本文件中的 `app`（Flask 实例）
# 作为 WSGI 应用接管请求，无需也不应手动启动开发服务器。
